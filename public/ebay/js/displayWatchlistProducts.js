// ------------------- Imports -------------------
import { escapeHtml, formatMoney, getProductImages } from './ebayCatalog.js';
...
const watchlistCategoryBtn = (category, count) => {
	...
	return `
		<button class="btn btn-sm px-3 btn-outline-secondary rounded-5 fs-8 text-capitalize" type="button" data-category="${safeCategory}">
        ${safeCategory} <span class="pe-none">(${Number(count) || 0})</span>
		</button>
	`;
};
...
const watchlistProductTemplate = (product) => {
	const {
		name,
		condition = 'Pre-owned',
		price,
	...
	const oldPrice = numericPrice * (numericDiscount / 100) + numericPrice;
	...
	return `<div class="d-flex gap-3 align-items-start border-bottom py-3">
              ...
              <div class="col-2 align-self-center" style="height: 150px;"> 
                <img src="${escapeHtml(thumbnail)}" alt="Product Image" class="img-fluid rounded-start border border-2 border-secondary border-opacity-25 object-cover w-100 h-100">
							</div>
							<div class="">
								<div class="">
                  <h3 class="fs-5 text-secondary mb-1">${escapeHtml(name ?? 'Untitled item')}</h3>
                  <p class="fs-8 text-secondary-emphasis">Condition: <span>${escapeHtml(condition)}</span></p>
								</div>
                          ...
                          <span class="fw-bolder">${formatMoney(numericPrice, currency)}</span>
													<div class="info-price-other fs-9">
                            <span class="text-decoration-line-through">${formatMoney(oldPrice, currency)}</span>
                            |<span class="fw-bolder ms-1">${numericDiscount}% OFF</span>
                          </div>
                            <span class="text-secondary-emphasis fs-8">+${formatMoney(numericShipping, currency)}</span>
												</div>
											</li>
											<li class="time-info">
												<div class="d-flex flex-column border-end pe-4">
                          ...
                          <a href="#" role="button" class="fs-8 cursor-pointer text-decoration-underline">${escapeHtml(seller)}</a>
                          <span class="fs-9 text-secondary-emphasis">${escapeHtml(sellerFeedback)}</span>
												</div>
											</li>
										</ul>
									</div>
